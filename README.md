# PORTAFOLIO-PERSONAL-GRUPO-6-
# Portafolio Personal

## Información del Grupo

* Grupo: 6
* Integrantes:

  * Jesús Loor
  * María Briones
  * Vicky Moran
  * Aarón Párraga
* Carrera: Ciencias de la Computación
* Curso: Tercer Semestre

## Sobre Nosotros

Somos estudiantes de la carrera de Ciencias de la Computación con interés en el desarrollo de software y la creación de soluciones tecnológicas. Nos enfocamos en aprender y aplicar conocimientos en programación, bases de datos y desarrollo de sistemas.

## Proyectos / Habilidades

* Sistema de Inventarios para empresa
* Desarrollo de aplicaciones web
* Programación en Python
* Manejo de bases de datos (SQL)
* Trabajo en equipo y desarrollo de proyectos

## Objetivo del Portafolio

Este repositorio tiene como finalidad presentar nuestras habilidades, conocimientos y proyectos desarrollados durante nuestra formación académica, sirviendo como una muestra de nuestro aprendizaje y crecimiento profesional.
